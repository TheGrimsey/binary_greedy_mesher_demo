...
use crate::utils::world_to_chunk;
...
fn iter_chunks_around(
    ...
    (-r..r).flat_map(move |x| {
        (-v_r..v_r).flat_map(move |y| (-r..r).map(move |z| IVec3::new(x, y, z) + center))
    })
}

fn update_chunk_pos(mut query: Query<(&GlobalTransform, &mut ChunkPos), Changed<GlobalTransform>>) {
...
pub fn scan<T: Send + Sync + Default + 'static>(
    ...
    // Update the global collector.
    {
        ...
        for (scanner, chunk_pos) in scanners.iter() {
            current_desired_chunks.extend(iter_chunks_around(
                chunk_pos.0,
...

    {
        ...
        gained_relevance_events.write_batch(newly_desired_chunks.into_iter().map(|&chunk| {
            ChunkGainedScannerRelevance {
                chunk,
                phantom_data: PhantomData,
            }
        }));
...

    {
        ...
        let no_longer_desired_chunks = global_desired_chunks
            .chunks
            .difference(&current_desired_chunks);
        lost_relevance_events.write_batch(no_longer_desired_chunks.into_iter().map(|&chunk| {
            ChunkLostScannerRelevance {
                chunk,
                phantom_data: PhantomData,
            }
        }));
    ...
    std::mem::swap(
        &mut global_desired_chunks.chunks,
        &mut current_desired_chunks,
    );
}
